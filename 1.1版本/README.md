...
1.1 版本新增：历史固定、取消固定和清空未固定记录；收起功能拆分为当前可见回答和全页回答；轮盘按钮增加禁用态和更明确的提示。
...
1.1 版本新增：下载入口移到帖子操作栏；新增立即下载/添加到队列流程；队列最多 30 项，并按图片、普通视频、GIF 视频分别处理下载、抽音频和转 GIF。
...
1.1 版本新增：新增立即下载/添加到队列流程；队列最多 30 项；继续通过公共 API 补全图片、普通视频和 GIF 视频信息。
