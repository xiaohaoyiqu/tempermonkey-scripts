...
Firefox、Violentmonkey 以及移动端浏览器需自行认证。
...
| [`1.1版本/`](./1.1%E7%89%88%E6%9C%AC/) | 1.1.0版本的文档。         |
| [`1.0版本/`](./1.0%E7%89%88%E6%9C%AC/) | 1.0.0版本的文档。         |
...
- 为已加载帖子提供帖子级下载按钮，并保留获取当前页面加载的帖子内容版本和按钮。
...
Bluesky 需要允许访问以下域名，也就是第一次使用时要追加权限：
...
Instagram 和 Bluesky 的 MP3 / GIF 功能会按需从 CDN 加载 `lamejs` 和 `gif.js`。第一次需要点击总是允许，如果拒绝跨源权限，API 补全、媒体读取或转换功能可能失败。
